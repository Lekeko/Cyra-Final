...
public class RoseCrystalFeature extends Feature<RoseCrystalFeatureConfig> {
    public RoseCrystalFeature(Codec<RoseCrystalFeatureConfig> configCodec) {
        super(configCodec);
    }

    @Override
    public boolean generate(FeatureContext<RoseCrystalFeatureConfig> context) {
        StructureWorldAccess world = context.getWorld();
        BlockPos origin = context.getOrigin();

        Random random = context.getRandom();
        RoseCrystalFeatureConfig config = context.getConfig();
        ...
        if (world.getRandom().nextBetween(1, 100) < 25){
            for (int y = 0; y < world.getHeight(); y++) {
                testPos = testPos.up();
                if ((world.getBlockState(testPos).isOf(ModBlocks.SEA_STONE) || world.getBlockState(testPos).isOf(ModBlocks.SEA_STONE_GRASS)) && world.getBlockState(testPos.up()).isOf(Blocks.WATER)) {

                    world.setBlockState(testPos.up().up(), ModBlocks.ROSE_CRYSTAL.getDefaultState(), 10);

                    for (int i = -1; i <= 1; i++)
                        for (int j = -1; j <= 1; j++) {
                            BlockPos pos = new BlockPos(testPos.up().getX() + i, testPos.up().getY(), testPos.up().getZ() + j);
                            world.setBlockState(pos, ModBlocks.SEA_MURIANITE.getDefaultState(), 10);
                        }
                    ...
                    for (int i = -radius; i <= radius; i++)
                        for (int j = -radius; j <= radius; j++) {
                            if (i * i + j * j <= radius * radius && world.getRandom().nextBetween(1, 14) > Math.abs(i) + Math.abs(j)) {
                                ...
                                world.setBlockState(tempPos, ModBlocks.SEA_MURIANITE.getDefaultState(), 10);
                            }
                    ...
                    for (int i = 0; i < fishes; i++){
                        ...
                        if (world.getBlockState(pos).isOf(Blocks.WATER)){
                            SeaRodentEntity entity = new SeaRodentEntity(ModEntities.SEA_RODENT, world.toServerWorld());
